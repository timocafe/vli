...
template <typename CoeffType> struct monomial {
  std::size_t j_exp;
  std::size_t h_exp;
  CoeffType coeff;
...
template <typename CoeffType, unsigned int Order> class polynomial;

/**
 * Multiplication of two polynomials
 */
template <typename CoeffType, unsigned int Order>
polynomial<CoeffType, 2 * Order>
operator*(polynomial<CoeffType, Order> const &p1,
          polynomial<CoeffType, Order> const &p2) {
  static std::size_t const order_cutoff =
      polynomial<CoeffType, Order>::order_cutoff;
    ...
    for (std::size_t he1 = 0; he1 <= Order; ++he1)
      for (std::size_t je2 = 0; je2 <= Order; ++je2)
        for (std::size_t he2 = 0; he2 <= Order; ++he2)
          result_coeffs[(je1 + je2) * (2 * Order + 1) + he1 + he2] +=
              p1.coeffs[je1 * order_cutoff + he1] *
              p2.coeffs[je2 * order_cutoff + he2];
...
polynomial<CoeffType, Order> operator*(polynomial<CoeffType, Order> const &p,
                                       monomial<T> const &m) {
...
public:
  ...
  friend polynomial<CoeffType, 2 * Order>
  operator*<>(polynomial<CoeffType, Order> const &,
              polynomial<CoeffType, Order> const &);
   ...
   */
  polynomial() : coeffs(order_cutoff * order_cutoff, CoeffType(0)) {}

  /**
   * Copy constructor
   */
  polynomial(polynomial const &p) : coeffs(p.coeffs) {}

  explicit polynomial(std::vector<CoeffType> const &v) : coeffs(v) {
    ...
    for (std::size_t je = 0; je < order_cutoff; ++je)
      for (std::size_t he = 0; he < order_cutoff; ++he)
        if (coeffs[je * order_cutoff + he] != CoeffType(0))
          o << "+" << coeffs[je * order_cutoff + he] << "*J^" << je << "*h^"
            << he << std::endl;
  }
   ...
   */
  template <typename T> polynomial &operator+=(monomial<T> const &m) {
  ...
  //                    p.coeffs[ (je+m.j_exp)*max_order + he+m.h_exp ] =
  //                    m.coeff * this->coeffs[ je*max_order + he ];
  //            swap(p);
  //            return *this;
  //        }
   ...
   */
  template <typename T> polynomial &operator*=(T const &c) {
    for (typename std::vector<CoeffType>::iterator it = coeffs.begin();
         it != coeffs.end(); ++it)
      *it *= c;
    return *this;
  }
  ...
  bool operator==(CoeffType const &c) const {
    if (coeffs[0] == c) {
      bool all_zero = true;
      for (typename std::vector<CoeffType>::const_iterator it = coeffs.begin();
           it != coeffs.end(); ++it)
   ...
   */
  inline CoeffType const &operator()(unsigned int j_exp,
                                     unsigned int h_exp) const {
    assert(j_exp <= Order);
    assert(h_exp <= Order);
...
std::ostream &operator<<(std::ostream &o,
                         polynomial<CoeffType, Order> const &p) {
  p.print(o);
  return o;
}
 ...
 */
template <typename CoeffType, unsigned int Order, typename T>
polynomial<CoeffType, Order> operator*(monomial<T> const &m,
                                       polynomial<CoeffType, Order> const &p) {
  return p * m;
}
 ...
 */
template <typename CoeffType, unsigned int Order, typename T>
polynomial<CoeffType, Order> operator*(polynomial<CoeffType, Order> p,
                                       T const &c) {
  p *= c;
  return p;
}
...
template <typename CoeffType, unsigned int Order, typename T>
polynomial<CoeffType, Order> operator*(T const &c,
                                       polynomial<CoeffType, Order> const &p) {
  return p * c;
}
...
template <typename CoeffType, unsigned int Order>
polynomial<CoeffType, 2 * Order>
inner_product(std::vector<polynomial<CoeffType, Order>> const &a,
              std::vector<polynomial<CoeffType, Order>> const &b) {
...
#pragma omp parallel for
  ...
  typename std::vector<polynomial<CoeffType, Order>>::const_iterator it(
      a.begin()),
      it_b(b.begin());
  while (it != a.end())
    result += *(it++) * *(it_b++);
