...
typedef boost::mpl::vector<
    ...
    vli::polynomial<int, vli::max_order_each<4>, var<'x'>, var<'y'>, var<'z'>,
                    var<'a'>>,
...
struct reference_data_storage {
  reference_data_storage() {
    ...
    ref_file.close();
    std::cout << ref_strings.size() << " reference data sets loaded."
              << std::endl;
  }
  template <class TestType>
  std::string get_result(unsigned int test_index, TestType type) const {
    typedef
        typename boost::mpl::find<polynomial_types, TestType>::type mpl_iter;
    unsigned int type_index = mpl_iter::pos::value;
    std::size_t index =
        test_index * boost::mpl::size<polynomial_types>::value + type_index;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(converting_copy_constructor, Poly,
                              polynomial_types) {
  typedef vli::polynomial<double, typename Poly::max_order,
...
BOOST_AUTO_TEST_CASE_TEMPLATE(plus_assign_zeroth_order, Poly,
                              polynomial_types) {
  Poly pa;
  fill_polynomial(pa);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(plus_assign_monomial, Poly, polynomial_types) {
  ...
  vli::monomial<typename Poly::value_type,
                typename vli::variable<Poly, 0>::type>
      ma(2);
  ma *= va;
  pa += ma;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(minus_assign_zeroth_order, Poly,
                              polynomial_types) {
  Poly pa;
  fill_polynomial(pa);
  Poly pb(pa);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(multiplies_assign_constant, Poly,
                              polynomial_types) {
  Poly pa;
  fill_polynomial(pa);
  Poly pb(pa);
...
// BOOST_AUTO_TEST_CASE_TEMPLATE (multivar_monomial_multiply, Poly,
// polynomial_types ) {
//    if(vli::num_variables<Poly>::value >= 2) {
//        Poly pa;
//        fill_polynomial(pa);
//        // test variables in reverse order:
//        vli::monomial<typename Poly::value_type, typename
//        vli::variable<Poly,1>::type, typename vli::variable<Poly,0>::type>
//        m(2,1); m *= 3; pa *= m; std::stringstream pa_str; pa_str << pa;
