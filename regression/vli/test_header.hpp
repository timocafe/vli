...
#define VLI_FUZZABLE_TEST(TEST_NAME)                                           \
  ...
  BOOST_AUTO_TEST_CASE(TEST_NAME) {                                            \
    std::cout << "Running " << vli::test::fuzz_initializer::fuzz_iterations    \
              << " iterations..." << std::endl;                                \
    for (unsigned int i = 0; i < vli::test::fuzz_initializer::fuzz_iterations; \
         ++i)                                                                  \
...
struct initializer {
  void operator()(integer_type &v, variant_enum variant = overflow_safe) {
    ...
    case multiplies_overflow_safe:
      ...
      for (std::size_t i = integer_type::numwords / 2;
           i != integer_type::numwords; ++i)
        v[i] = 0;
      break;
    case fill_ff:
      break;
  ...
  void operator()(integer_type_double &v,
                  variant_enum variant = overflow_safe) {
    for (std::size_t i = 0; i != integer_type_double::numwords; ++i)
      v[i] = std::numeric_limits<integer_type::value_type>::max();
    switch (variant) {
    case max_positive:
      v[integer_type_double::numwords - 1] =
          v[integer_type_double::numwords - 1] &
          ~(integer_type::value_type(1)
            << (sizeof(integer_type_double::value_type) * 8 - 1));
      break;
    case overflow_safe:
      v[integer_type_double::numwords - 1] = 1;
      break;
    case multiplies_overflow_safe:
      v[integer_type_double::numwords / 2 - 1] =
          v[integer_type_double::numwords / 2 - 1] & 0x7fffffffffffffff;
      for (std::size_t i = integer_type_double::numwords / 2;
           i != integer_type_double::numwords; ++i)
        v[i] = 0;
      break;
    default:
      throw(std::runtime_error("init variant not implemented"));
...
struct fuzz_initializer {
  ...
  static std::uniform_int_distribution<integer_type::value_type>
      integer_value_type_max_rnd;
  ...
  void operator()(integer_type &v, variant_enum variant = overflow_safe) {
    ...
    switch (variant) {
    case max_positive:
      v[integer_type::numwords - 1] =
          v[integer_type::numwords - 1] &
          ~(integer_type::value_type(1)
            << (sizeof(integer_type::value_type) * 8 - 1));
      break;
    case overflow_safe:
      v[integer_type::numwords - 1] = 1;
      break;
    case multiplies_overflow_safe:
      v[integer_type::numwords / 2 - 1] =
          v[integer_type::numwords / 2 - 1] & 0x7fffffffffffffff;
      for (std::size_t i = integer_type::numwords / 2;
           i != integer_type::numwords; ++i)
        v[i] = 0;
...
std::uniform_int_distribution<integer_type::value_type>
    fuzz_initializer::integer_value_type_max_rnd =
        std::uniform_int_distribution<integer_type::value_type>(
            0, std::numeric_limits<integer_type::value_type>::max());
std::uniform_int_distribution<int> fuzz_initializer::int_plus_rnd =
    std::uniform_int_distribution<int>(0, std::numeric_limits<int>::max());
#endif // VLI_FUZZ_TESTS

template <typename T> struct extended {};

template <std::size_t NumBits> struct extended<integer<NumBits>> {
  typedef integer<NumBits + 64> type;
};

template <typename T> struct double_sized_integer {};

template <std::size_t NumBits> struct double_sized_integer<integer<NumBits>> {
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1 && argc <= 3) {
    vli::test::fuzz_initializer::fuzz_iterations =
